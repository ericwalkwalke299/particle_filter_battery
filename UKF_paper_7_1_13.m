...
first_discharge_current = B0006.cycle(1,4).data.Current_measured...
    (3:end);

second_discharge_voltage = B0006.cycle(1,2).data.Voltage_measured...
    (3:end);
second_discharge_time    = B0006.cycle(1,2).data.Time(3:end);
second_discharge_current = B0006.cycle(1,2).data.Current_measured...
    (3:end);
...
for i=1:length(second_discharge_time)
    ...
    U_p      = ( -4.656 + 88.669 * x_psurf.^2 - 401.119 *...
        x_psurf.^4 + 342.909 * ...
        x_psurf.^6 - 462.471 * x_psurf.^8 + 433.434 *...
        x_psurf.^10 ) ./ ...
    ...
    eta_n    = R_g * T / (F * alpha_a) * log( ( J_n + (-4*c_e*...
       ...
       + 4*c_e*F^2*c_nmax^2*k_n^2.*x_nsurf+J_n.^2).^0.5 ) ./ (2*...
       F*c_e^0.5*k_n*(c_nmax*x_nsurf).^0.5 .* ...
       (c_nmax-c_nmax.*x_nsurf).^0.5) ) ;
   
    eta_p    = R_g * T / (F * alpha_c) * log( ( J_p + (-4*c_e*...
        F^2*c_pmax^2*k_p^2.*x_psurf_set.^2 ...
       + 4*c_e*F^2*c_pmax^2*k_p^2.*x_psurf_set+J_p.^2).^0.5 ) ./...
       (2*F*c_e^0.5*k_p.*(c_pmax.*x_psurf_set).^0.5 .* ...
       (c_pmax-c_pmax.*x_psurf_set).^0.5) );
   
    zeta = real(U_p + eta_p - U_n - eta_n);
    y_hat    = sum(W.*zeta);
...
plot(second_discharge_time(1:length(y_hat_storage)), y_hat_storage,...
    'k-', 'linewidth', 2)
plot(second_discharge_time, second_discharge_current, 'ko')
axis([0 4300 2.4 4])
xlabel('Time (s)')
ylabel('Voltage (V)')
legend('UKF prediction 928.6 (s)', 'observations')
