...
for i=1:(stop_cycle-29)
    ...
    weights         = 1/(0.02*sqrt(2*pi)).*exp(-(V_cell_set(i,:)...
        -second_discharge_voltage(i)).^2/(2*0.02^2));
    weights   = weights/sum(weights) ;
...
expecSP = sum(weights_50.*second_discharge_time(EOD));
plot([expecSP, expecSP],[0,max(f)],'g')
legend('prediction pdf','observation','expected value of pdf')
